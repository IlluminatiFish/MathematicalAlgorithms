...
def dijkstrasAlgorithm(graph, start, end):
    ...
    while graph: # While graph exists, meaning until all the vertices have been passed through
        ...
        for vertex in graph: # Loop through each vertex in the graph
            ...
            elif distance[vertex] < distance[shortest]: # If the distance of the current vertex is smaller the then distance of the shortest vertex then set the shortest to the vertex thats been worked on
        ...
        for neighbour, length in graph[shortest].items(): # Caluclate the shortest distance of each edge
            ...
            if neighbour in graph and (length + distance[shortest] < distance[neighbour]): 
                ...
                distance[neighbour] = length + distance[shortest] # Add the length of the edge to the distance of the current shortest working path and set it to the distance of the neighbouring vertex
                previous_vertex[neighbour] = shortest # Set the previous neighbouring vertex to be the current shortest final vertex
